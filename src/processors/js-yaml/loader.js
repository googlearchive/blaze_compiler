...
/*********************************************************************************************************
The following was added to js-yaml to track the start and end of keys, values, arrays and objects
/*********************************************************************************************************/
var debug = false; //its hard to debug!!!
var Json = require("../Json.js");
var scope = []; //we mark positions as we descend into parsing, and pop on the way out
var entries=[]; //push and popped inline with scope, stores mappings so we can load them when the object is built
var entry_values=[]; //push and popped inline with scope, stores mappings so we can load them when the object is built
var entry_keys=[];   //push and popped inline with scope, stores mappings so we can load them when the object is built
var array_elements = []; //pushed and popped in sync with scope

var last_popped = null;
var suppress_next_element = false;
var log_indent = "";

var early_block_mapping_exit = false;

/**
 * When a new object or value or array is created, you call jstart()
 * When it finishes, you call jend(), which works out from js-yaml internals what type was actually created
 * during parsing, you indicate the last jend() object was a key jkey()
 * when an array element is parsed you indicate it with jelement()
 * finally the - block arrays are weird and occur an extra level of depth than would be obvious,
 * so call jelementBlock() to do a jelement() but at one level deeper than usual.
 * there is some serious confusion on how arrays are processed
 * @param state
 */
var log = function(msg) {
    console.log(log_indent, msg);
};

var jstart = function(state){
    if (debug) log("jstart");
    scope.push(new Mark(state.filename, state.input, state.position, state.line, (state.position - state.lineStart)));
    entries.push({});
    entry_values.push({});
    entry_keys.push({});
    array_elements.push([]);
...
var jend = function(state){
    var start = scope[scope.length-1].position;
    var mark_end = new Mark(state.filename, state.input, state.position, state.line, (state.position - state.lineStart));
    var end   = mark_end.position;

    var value = null;
    if(state.result == null) { //null has to be before object as it's typeof is 'object'
        value = new Json.JNull(start, end);
    } else if(typeof state.result === 'boolean') {
        value = new Json.JBoolean(state.result, start, end);
    } else if(typeof state.result === 'string') {
        value = new Json.JString(state.result, start, end);
    } else if(typeof state.result === 'number') {
        value = new Json.JNumber(state.result, start, end);
    } else if(Array.isArray(state.result)) {  //array has to be before object as it's typeof is 'object'
        value = new Json.JArray();
        value.setStart(start);
        value.setEnd(end);

        //when an array is built we have to populate it with the array data we already parsed in the same scope
        for (var element_id in array_elements[array_elements.length-1]) {
            var element = array_elements[array_elements.length-1][element_id];
            value.push(element);
        }
        if (debug) log("jarray " + JSON.stringify(value.toJSON()));
    } else if(typeof state.result === 'object') {
        value = new Json.JObject();
        value.setStart(start);
        value.setEnd(end);

        //when an object is built we have to populate it with the key-value data we already parsed in the same scope
        for (var key_string in entry_keys[entry_keys.length-1]) {
            var key_node = entry_keys[entry_keys.length-1][key_string];
            var val_node = entry_values[entry_values.length-1][key_string];
            value.put(key_node, val_node);
        }
        if (debug) log("jobject " + JSON.stringify(value.toJSON()));
    }

    if(value == null){
        throw new Error("unexpected result in js-yaml adapter");
    }

    if (debug) log("jend " + Json.JType[value.type]);

    value.setStart(scope[scope.length-1].position);
    value.setEnd(mark_end);
    ...
    last_popped = value;
    module.exports.last_popped = last_popped; //make it visiable outside


    if (debug && typeof state.result === 'string' && state.result == "\\$.+") {
        log("break here for conditional debugging")
    }

    log_indent = log_indent.slice(0, log_indent.length - 2);

    return value;
};

var jend_early = function (state) {
    //pop scope but don;t use the result
    scope.pop();
    entries.pop();
    entry_values.pop();
    entry_keys.pop();
...
/**
 * indicate the last popped value was a key in a dictionary
 */
var jkey = function() {
    entries[entries.length-1][last_popped.value] = last_popped;
};

/**
 * indicated the last popped value was an element of an array
 */
var jelement = function() {
    if (!suppress_next_element){
        array_elements[array_elements.length-1].push(last_popped);
    } else {
        suppress_next_element = false;
    }
};

var jelementBlock = function() {
    suppress_next_element = true;
    array_elements[array_elements.length-2].push(last_popped);
};

/**
 * to add a key-value to an object
 */
var jput = function(state, key, value){
    //empirically, the value node is the last popped object
    var value_node = last_popped;
    //the key is a string, to ensure we captured the string, all strings were pushed to entries map with null
    var key_node = entries[entries.length-1][key];
    //add a proper entry of a JString -> JValue
    entry_keys[entries.length-1][key] = key_node;
    entry_values[entries.length-1][key] = value_node;

    if (debug) log("jput", key, value);
};

var jputBlock = function(state, key, value){
    //empirically, the value node is the last popped object
    var value_node = last_popped;
    //the key is a string, to ensure we captured the string, all strings were pushed to entries map with null
    var key_node = entries[entries.length-2][key];
    //add a proper entry of a JString -> JValue
    entry_keys[entries.length-2][key] = key_node;
    entry_values[entries.length-2][key] = value_node;

    if (debug) log("jput", key, value);
};

/*********************************************************************************************************
End the stuff we added to js-yaml
/*********************************************************************************************************/
...
function storeMappingPair(state, _result, keyTag, keyNode, valueNode) {
  ...
  //state.position will be set to the end of this key -> value pair
...
function readFlowCollection(state, nodeIndent) {
  if (debug)  log(">readFlowCollection");
  var readNext = true,
      _line,
      _tag     = state.tag,
      _result,
  ...
  default:
    if (debug)  log("<readFlowCollection false");
    return false;
  }
  ...
  while (state.position < state.length) {
    ...
    if (state.character === terminator) {
      ...
      state.result = _result;
      if (debug)  log("<readFlowCollection true");
      return true;
    } else if (!readNext) {
    ...
    if (isMapping) {
      ...
      _result.push(storeMappingPair(state, null, keyTag, keyNode, valueNode)); //todo is this an array?
    } else {
      _result.push(keyNode); //this is a single element in a inline [ ] block
      jelement();
    }
...
function readBlockSequence(state, nodeIndent) {
  if (debug)  log(">readBlockSequence");
  var _line,
      _tag      = state.tag,
      _result   = [],
      following,
  ...
  if (detected) {
    ...
    state.kind = 'sequence';
    state.result = _result;
    if (debug)  log("<readBlockSequence true");
    return true;
  } else {
    if (debug)  log("<readBlockSequence false");
    return false;
  }
}

function readBlockMapping(state, nodeIndent) {
  if (debug)  log(">readBlockMapping");
  early_block_mapping_exit = false;
  var following,
      allowCompact,
      _line,
      _tag          = state.tag,
    ...
    //
      ...
      if (state.line === _line) {
        ...
        if (CHAR_COLON === state.character) {
          ...
          state.tag = _tag;
          //this seems to happen when the element is on the next line, whether an array or an object
          if (debug)  log("<readBlockMapping true early 1 " + state.tag);
          early_block_mapping_exit = true;
        ...
        state.tag = _tag;
        if (debug)  log("<readBlockMapping true early 2");
        return true; // Keep the result of `composeNode`.
      }
    ...
    if (state.line === _line || state.lineIndent > nodeIndent) {
      if (composeNode(state, nodeIndent, CONTEXT_BLOCK_OUT, true, allowCompact)) {
        if (atExplicitKey) {
          ...
          valueNode = state.result; //an array assignment
        }
      }
  ...
  if (debug)  log("<readBlockMapping end " + detected);
  return detected;
}
...
function composeNode(state, parentIndent, nodeContext, allowToSeek, allowCompact) {
  ...
  if (allowToSeek) {
    if (skipSeparationSpace(state, true, -1)) {
      ...
      if (state.lineIndent === parentIndent) {
        ...
        if (debug) log("exit composeNode false - no jend");
        return false;
      }
    }
  }
    ...
    while (readTagProperty(state) || readAnchorProperty(state)) {
      if (skipSeparationSpace(state, true, -1)) {
        ...
        if (state.lineIndent > parentIndent) {
          ...
          if (debug) log("exit composeNode true - no jend");
          return true;
        }
      } else {
  ...
  var result = null !== state.tag || null !== state.anchor || hasContent;
  if (debug) log("exit composeNode with " + result);
  return result;
}
